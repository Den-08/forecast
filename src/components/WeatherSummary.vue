...
const today = new Date().toLocaleString('en-EN', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric'});
</script>

<template>
  <div v-if="weatherInfo?.weather" class="summary">
    <div
      v-bind:style="`background-image: url('/src/assets/img/weather-main/${weatherInfo?.weather[0]?.description}.png');`"
      class="pic-main"></div>
    <div class="weather">
    ...
    <div class="city text-block">{{ capitalizeFirstLetter(weatherInfo?.name) }}, {{ weatherInfo?.sys.country }}</div>
